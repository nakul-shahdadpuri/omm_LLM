...
type DocumentWithContent = DocumentMeta & {
  content?: string;
};
...
export default function ResumeQAApp() {
  ...
  const [selectedDoc, setSelectedDoc] = useState<DocumentWithContent | null>(null);
  ...
  const formatAnswer = (text: string, source: string) => {
    ...
    return parts
      .map((part, idx) => {
        if (/^\d+\.\s+\*\*[^*]+\*\*$/.test(part)) {
          return (
            <h4 key={idx} className="text-md font-semibold mt-4">
              {part.replace(/\*\*/g, "")}
            </h4>
          );
        } else if (/^\*\*[^*]+\*\*$/.test(part)) {
          return (
            <strong key={idx} className="block font-medium mt-2">
              {part.replace(/\*\*/g, "")}
            </strong>
          );
        } else if (/\[.*?\]\((.*?)\)/.test(part)) {
      ...
      .concat(
        source
          ? [
              <p key="source" className="text-xs text-gray-400 border-t pt-2">
                📄 Source: <em>{source}</em>
              </p>,
            ]
          : []
  ...
  return (
    ...
    >
      <header
        className={`p-4 border-b shadow text-xl sticky top-0 z-10 flex items-center justify-between ${
          darkMode
      ...
      <div className="flex flex-1">
        <div
          className={`w-1/4 min-w-[240px] p-4 border-r overflow-hidden flex flex-col ${
        ...
        <div className="flex-1 p-6 flex flex-col">
          <div
            className={`flex-grow border rounded-xl p-5 mb-4 overflow-auto leading-relaxed ${
        ...
        {selectedDoc && (
          <div
            className={`w-1/3 p-4 border-l overflow-auto ${
